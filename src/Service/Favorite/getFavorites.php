...
{
    $dbh = getConnexion();
    $sth = $dbh->prepare("SELECT favorite.id, favorite.href, favorite.title, favorite.description FROM user_favorite JOIN favorite ON user_favorite.favorite_id = favorite.id WHERE user_favorite.user_id = :id ORDER BY id DESC;");
